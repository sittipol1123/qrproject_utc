<?php    
    ...
    if (isset($_REQUEST['data'])) { 
        ...
        $parcelid = $_POST["id"];
        session_start();
        mysql_connect("qr.utc.ac.th","admin_qrdb","c8U4hO5kqO");
        mysql_select_db("admin_qrDB");
        ...
        if ($stus == "ADMIN") {
            echo "<script>
                    alert('สำเร็จ');
                    window.location ='../show_detail.php?parcelID=".$parcelid."';
                </script>";

        }else{ 
            echo "<script>
                        alert('สำเร็จ');
                        window.location ='../employee/show_detail.php?parcelID=".$parcelid."';
                    </script>";
         }
